...
static double time_spec_diff(struct timespec &start, struct timespec &end) {
...
static void readairbin(std::ifstream &infile, uint8_t column_offset) {
  ...
  std::cout << std::hex << std::setw(8) << std::setfill(' ') << "Tile Off"
            << ' ';
...
static uint64_t airbin2mem(std::ifstream &infile, volatile uint32_t *tds_va,
